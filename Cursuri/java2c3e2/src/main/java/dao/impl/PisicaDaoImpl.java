...
public class PisicaDaoImpl implements PisicaDao {
    ...
    public void remove(Pisica p) {
        entityManager.getTransaction().begin();
        entityManager.remove(p);// p  trebuie sa faca parte din contextrul de persistenta
        entityManager.getTransaction().commit();
    }

    @Override
    public Pisica refresh(Pisica p) {
        //actualizeaza instatna cu ce exista in baza de date
    ...
    public Pisica merge(Pisica p) {
        //punem instanta in context
        return entityManager.merge(p);
    }

    @Override
    public void detach(Pisica p) {
        //ii spunem EntityManager-ului sa nu mai administreze instanta.
    ...
    @Override
    public void clear() {
        // detasam toate instatnatele administrate de acest entity manager
        entityManager.clear();
